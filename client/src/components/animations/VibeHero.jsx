...
/* ═══════════════════════════════════════════════════
   VIBE HERO — GSAP MASTER TIMELINE
   One timeline. Zero loops. Every frame scroll-driven.
   ═══════════════════════════════════════════════════ */
export default function VibeHero() {
  // DOM refs — every animated element is a plain div
  ...
  // Mouse tilt via GSAP (not Framer Motion)
  ...
  // ── MASTER TIMELINE ──
  useLayoutEffect(() => {
    // Measure route path length for stroke animation
    ...
    const ctx = gsap.context(() => {
      ...
      /* ═══ STEP 2: CAMERA PUSH (0.7 → 1.8s) ═══ */
      tl.to(phoneRef.current,
        { scale: 1.2, y: -20, duration: 1.1, ease: 'power2.inOut' },
        0.7 // overlap with step 1
      );
      // Text exits
      ...
      // Ambient glow appears softly
      tl.to(glowRef.current,
        { scale: 1.2, opacity: 0.5, duration: 1, ease: 'power2.inOut' },
        1.0
      );

      /* ═══ STEP 3: MAP MOVEMENT (1.5 → 4.5s) ═══ */
      tl.to(mapLayerRef.current,
        { y: -180, duration: 3, ease: 'power2.inOut' },
        1.5
      );

      /* ═══ STEP 4: ROUTE DRAW (1.8 → 4.0s) ═══ */
      ...
      /* ═══ STEP 5: CAR APPEARS + FOLLOWS PATH (2.0 → 4.5s) ═══ */
      // Car fades in
      ...
      // Nav bar appears
      tl.to(uiNavRef.current,
        { y: 0, opacity: 1, duration: 0.5, ease: 'power2.out' },
        2.2
      );
      // Car follows the SVG route path
      tl.to(carRef.current, {
        motionPath: {
          path: ROUTE_PATH,
          align: 'self',
      ...
      // Phone continues subtle rotation during journey
      tl.to(phoneRef.current,
        { scale: 1.3, rotateX: 6, rotateY: -2, duration: 2.5, ease: 'power2.inOut' },
        2.0
      );
      // Edge light sweep during journey
      tl.to(lightRef.current,
        { x: '150%', duration: 2, ease: 'power2.inOut' },
        2.5
      );

      /* ═══ STEP 6: ARRIVAL (4.3 → 5.2s) ═══ */
      // Destination node pulses once
      tl.to(destNodeRef.current,
      ...
      /* ═══ STEP 7: UI CARD REVEAL (4.8 → 5.6s) ═══ */
      tl.to(uiCardRef.current,
        { y: '0%', opacity: 1, duration: 0.8, ease: 'power2.inOut' },
        4.8
      );

      /* ═══ STEP 8: SETTLE (5.5 → 6.5s) ═══ */
      tl.to(phoneRef.current,
    ...
    >
      ...
      <div className="relative z-10 h-full max-w-[1300px] mx-auto px-8 flex items-center">
        <div className="w-full grid lg:grid-cols-[1fr_auto] gap-16 items-center">
          ...
          {/* LEFT: TEXT (GSAP-controlled exit) */}
          <div ref={textRef} className="max-w-lg" style={{ willChange: 'transform, opacity, filter' }}>
            <div className="space-y-8">
              ...
              <h1 className="text-[3.5rem] md:text-[4rem] lg:text-[4.5rem] font-semibold text-white tracking-[-0.03em] leading-[1.08]">
                Find Parking<br />
                <span className="text-[#0A84FF]">Near You</span><br />
                Anytime
              </h1>
          ...
          {/* RIGHT: 3D PHONE (GSAP-controlled) */}
              ...
              <div className="relative w-[280px] h-[580px] md:w-[295px] md:h-[620px]">
                ...
                <div className="absolute inset-0 rounded-[2.8rem] bg-gradient-to-b from-[#2c2c2e] via-[#1c1c1e] to-[#161618] border border-white/[0.05] shadow-[0_35px_70px_-18px_rgba(0,0,0,0.75)]">
                  ...
                  <div className="absolute inset-[4px] rounded-[2.6rem] overflow-hidden bg-[#1c1c1e]">
                    ...
                    {/* LAYER 0: MAP (GSAP translateY) */}
                    <div ref={mapLayerRef} className="absolute inset-x-0 top-0 w-full h-[170%]"
                      style={{ willChange: 'transform', filter: 'blur(0.2px)' }}>
                      <svg className="w-full h-full" viewBox="0 0 300 850" preserveAspectRatio="none">
                        ...
                        {[
                          ...
                          <rect key={i} x={x} y={y} width={w} height={h} rx="2.5"
                            fill="rgba(255,255,255,0.015)" stroke="rgba(255,255,255,0.022)" strokeWidth="0.5" />
                        ))}
                        {/* Roads */}
                        {[120,260,400,540,680].map((y,i) => (
                          <line key={`h${i}`} x1="0" y1={y} x2="300" y2={y}
                            stroke="rgba(255,255,255,0.035)" strokeWidth="16" />
                        ))}
                        {[85,180,245].map((x,i) => (
                          <line key={`v${i}`} x1={x} y1="0" x2={x} y2="850"
                            stroke="rgba(255,255,255,0.035)" strokeWidth="14" />
                        ))}
                      </svg>
                    </div>

                    {/* LAYER 1: ROUTE (GSAP stroke-dashoffset) */}
                    <svg className="absolute inset-0 w-full h-full z-10" viewBox="0 0 300 560">
                      ...
                      {/* Route glow */}
                      <path ref={routeGlowRef} d={ROUTE_PATH} fill="none"
                        stroke="rgba(10,132,255,0.1)" strokeWidth="8" strokeLinecap="round"
                        filter="url(#rGlow)" />
                      {/* Route line */}
                      <path ref={routePathRef} d={ROUTE_PATH} fill="none"
                        stroke="rgba(10,132,255,0.5)" strokeWidth="2.5" strokeLinecap="round" />
                    </svg>

                    {/* LAYER 2: PARKING SPOTS (static) */}
                    <div className="absolute inset-0 z-20">
                      ...
                      {/* DESTINATION NODE (GSAP-animated) */}
                      <div className="absolute" style={{ left: 215, top: 155, transform: 'translate(-50%,-50%)' }}>
                        {/* Expansion ring */}
                        <div ref={destRingRef} className="absolute inset-0 flex items-center justify-center"
                          style={{ willChange: 'transform, opacity' }}>
                          <div className="w-10 h-10 rounded-full border border-[rgba(10,132,255,0.3)]" />
                        </div>
                        {/* Glow */}
                        <div ref={destGlowRef} className="absolute inset-0 flex items-center justify-center"
                          style={{ willChange: 'transform, opacity' }}>
                          <div className="w-8 h-8 rounded-full bg-[rgba(10,132,255,0.12)]" style={{ filter: 'blur(8px)' }} />
                        </div>
                        {/* Core marker */}
                        <div ref={destNodeRef} className="relative w-[10px] h-[10px] rounded-full"
                          style={{
                    ...
                    {/* LAYER 3: CAR (GSAP motionPath) */}
                    <div ref={carRef} className="absolute z-30 left-0 top-0"
                      style={{ willChange: 'transform, opacity' }}>
                      <svg width="20" height="30" viewBox="0 0 20 30" fill="none" className="block"
                    ...
                    {/* LAYER 4: UI OVERLAYS (GSAP-controlled) */}
                    {/* Nav bar */}
                    <div ref={uiNavRef} className="absolute top-12 left-3.5 right-3.5 z-40"
                      style={{ willChange: 'transform, opacity' }}>
                    ...
                    {/* Arrival card */}
                    <div ref={uiCardRef} className="absolute bottom-4 left-3 right-3 z-40"
                      style={{ willChange: 'transform, opacity' }}>
      ...
      {/* Scroll cue — part of initial state, hidden by scroll */}
